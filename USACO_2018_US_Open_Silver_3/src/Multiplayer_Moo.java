...
public class Multiplayer_Moo {
	...
	public static void main(String[] args) throws IOException{
		...
		int max_amount2 = 0;
		Set<String> repeat2 = new HashSet<>();
		for (int i = 0; i < graph.size(); i ++)
		{
			for (int j : graph.get(i))
			{
				if (!repeat2.contains(nums.get(j).get(0) + " " + nums.get(i).get(0)))
				{
					repeat2.add(nums.get(i).get(0) + " " + nums.get(j).get(0));
					cur_amount2 = 0;
					int num1 = nums.get(i).get(0);
					int num2 = nums.get(j).get(0);
					dfs2(num1, num2, i, new HashSet<>());
	...
	public static void dfs2(int num1, int num2, int i, Set<Integer> repeat) {
		repeat.add(i);
		cur_amount2 += nums.get(i).get(1);
		for (int k : graph.get(i))
		{
			if ((nums.get(k).get(0) == num1 || nums.get(k).get(0) == num2) && !repeat.contains(k))
			{
				dfs2(num1, num2, k, repeat);
			}
		}
	}
}
